...
function! ConvertToHex(colorcode) abort
  if a:colorcode =~ "#"
    return a:colorcode
  endif
  ...
  return l:colormappings[string(str2nr(a:colorcode))]
endfunction
...
function! GetColorValue(synID) abort
  ...
  return ConvertToHex(l:color)
endfunction

" Get some color values that are not picked up by GetColorValues
function! GetExtraColorValues() abort
  return {
        \ 'NormalFg':        ConvertToHex(synIDattr(hlID('Normal'),       'fg#')),
        \ 'NormalBg':        ConvertToHex(synIDattr(hlID('Normal'),       'bg#')),
        \ 'StatusLineFg':    ConvertToHex(synIDattr(hlID('StatusLine'),   'fg#')),
        \ 'StatusLineBg':    ConvertToHex(synIDattr(hlID('StatusLine'),   'bg#')),
        \ 'CursorFg':        ConvertToHex(synIDattr(hlID('Cursor'),       'fg#')),
        \ 'CursorBg':        ConvertToHex(synIDattr(hlID('Cursor'),       'bg#')),
        \ 'LineNrFg':        ConvertToHex(synIDattr(hlID('LineNr'),       'fg#')),
        \ 'LineNrBg':        ConvertToHex(synIDattr(hlID('LineNr'),       'bg#')),
        \ 'CursorLineFg':    ConvertToHex(synIDattr(hlID('CursorLine'),   'fg#')),
        \ 'CursorLineBg':    ConvertToHex(synIDattr(hlID('CursorLine'),   'bg#')),
        \ 'CursorLineNrFg':  ConvertToHex(synIDattr(hlID('CursorLineNr'), 'fg#')),
        \ 'CursorLineNrBg':  ConvertToHex(synIDattr(hlID('CursorLineNr'), 'bg#')),
        \  }
endfunction

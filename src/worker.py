...
def update_repository(repository, last_import_at):
    owner_name = repository["owner"]["name"]
    name = repository["name"]

    is_repository_new, old_repository = db_service.is_repository_new(owner_name, name)
...
def should_fetch_images(last_commit_at, last_import_at, is_repository_new):
    last_commit_at = (
    ...
    images_will_be_fetched = (
        is_repository_new or last_import_at is None or last_commit_at > last_import_at
    )
    printer.info(
        "Images will be fetched" if images_will_be_fetched else "Image fetching skipped"
    )
    return images_will_be_fetched


def fetch_images(repository, old_repository):
    image_urls = (
        list(
            filter(
    ...
    repository_image_urls = (
        github.list_repository_image_urls(repository, image_count_to_find, image_urls)
        if image_count_to_find > 0
        else []
    )
...
def create_import(elapsed_time):
    import_data = {
